...
//       return data.map((json) => DistributorModel.fromJson(json)).toList();
//     } else {
...
//     print('POST Response: $responseBody'); // Debug
//     if (response.statusCode != 200) {
...
//   Future<List<DistributorModel>> getDistributors(String type) async {
//     final uri = Uri.parse('$baseUrl/get_retailer_distributor_master/1?type=$type');
//     final response = await http.get(
//       uri,
//       headers: {
//         'Authorization': authToken,
...
//         // Check for a 'data' key containing the list
...
class ApiService {
  ...
  Future<List<DistributorModel>> getDistributors(String type) async {
    final uri = Uri.parse('$baseUrl/get_retailer_distributor_master/1');
    final response = await http.post( // Changed to POST to match Postman collection
      uri,
      headers: {
        'Authorization': authToken,
        'Content-Type': 'application/json',
    ...
    if (response.statusCode == 200) {
      ...
      // Filter by type locally if API doesn't
